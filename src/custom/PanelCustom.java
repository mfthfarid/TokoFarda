...
public class PanelCustom extends JPanel {
    ...
    protected void paintComponent(Graphics graphic) {
        ...
        if(roundTopRight>0){
            area.intersect(new Area(crateRoundTopRight()));
        }
        
        if(roundTopLeft>0){
            area.intersect(new Area(crateRoundBottomLeft()));
        }
        
        if(roundBottomRight>0){
            area.intersect(new Area(crateRoundBottomRight()));
        }
    ...
    private Shape crateRoundTopRight() {
        int width = getWidth();
        int height = getHeight();
        int roundX = Math.min(width, roundTopLeft);
        int roundY = Math.min(width, roundTopLeft);
    ...
    private Shape crateRoundBottomLeft() {
        int width = getWidth();
        int height = getHeight();
        int roundX = Math.min(width, roundTopLeft);
        int roundY = Math.min(width, roundTopLeft);
    ...
    private Shape crateRoundBottomRight() {
        int width = getWidth();
        int height = getHeight();
        int roundX = Math.min(width, roundTopLeft);
        int roundY = Math.min(width, roundTopLeft);
